...
# Page config
st.set_page_config(
    page_title="RAG Chatbot - Poslovni Asistent",
    page_icon="🤖",
    layout="centered"
)
...
# Main UI
def main():
    st.title("🤖 RAG Chatbot - Poslovni Asistent")
            ...
            if "sources" in message and message["sources"]:
                with st.expander("📄 Izvori"):
                    for source in message["sources"]:
                        st.write(f"• {source['filename']}, stranica {source['page']}")
    ...
    # Sidebar - Examples
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 💡 Primjeri pitanja")
